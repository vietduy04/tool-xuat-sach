...
from etl.process import process_chunk, preprocess_rules
from etl.validate import validate_chunk
from etl.output import write_chunk_to_csv, combine_csv_files
import config
    ...
    with st.sidebar:
        ...
        with st.expander("🔧 Advanced Configuration"):
            ...
            chunk_size = st.number_input(
                ...
                min_value=1000,
                max_value=1000000,
                value=st.session_state.config_data.get('chunk_size', config.DEFAULT_CHUNK_SIZE),
                step=10000,
...
def process_files(raw_files, separate_files: bool):
    ...
    try:
        ...
        output_folder = st.session_state.config_data.get('output_folder', config.DEFAULT_OUTPUT_FOLDER)
        ...
        chunk_size = st.session_state.config_data.get('chunk_size', config.DEFAULT_CHUNK_SIZE)
        ...
        if separate_files:
            ...
            combined_output_file = os.path.join(output_folder, f"Combined_{run_timestamp}.csv")
        ...
        for file_idx, raw_file in enumerate(raw_files):
            with st.status(f"Processing file {file_idx + 1}/{total_files}: {raw_file.name}...") as status:
                ...
                for chunk in pd.read_csv(csv_path, chunksize=chunk_size, header=None):
                    ...
                    st.session_state.progress = min(
                        (file_idx / total_files) + ((chunk_count * chunk_size) / (total_files * chunk_size * 10)),
                        1.0
                    )
        ...
        st.success(f"✅ Processing completed! Processed {st.session_state.total_rows} total rows.")
